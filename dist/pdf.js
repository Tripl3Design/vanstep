function createPdf(model, mainImage, title, fsid) {
    ...
    let accessoriesTableBody = [
        [{ text: t('onderdeel'), bold: true, fontSize: 10 }, { text: t('optie'), bold: true, fontSize: 10 }, { text: t('prijs'), bold: true, alignment: 'right', fontSize: 10 }]
    ];

    const P = window.PRICING_CONFIG;

    // Basisprijs
    accessoriesTableBody.push([t('basisprijs'), '', { text: `€ ${P.basePrice},-`, alignment: 'right', fontSize: 10 }]);

    // Meerprijzen Bus
    if (model.van) {
        if (model.van.brand === 'volkswagen' || model.van.brand === 'man') {
            accessoriesTableBody.push(['Meerprijs merk', `${model.van.brand.charAt(0).toUpperCase() + model.van.brand.slice(1)}`, { text: `€ ${P.van.brand_surcharge},-`, alignment: 'right', fontSize: 10 }]);
        }
        if (model.van.brand === 'mercedes') {
            if (model.van.lenght === 'L1') accessoriesTableBody.push(['Meerprijs lengte', 'L1', { text: `€ ${P.van.mercedes_l1},-`, alignment: 'right', fontSize: 10 }]);
            if (model.van.drive === 'fwd') accessoriesTableBody.push(['Meerprijs aandrijving', 'Voorwielaandrijving', { text: `€ ${P.van.mercedes_fwd},-`, alignment: 'right', fontSize: 10 }]);
        }
        if (model.van.rearWheel === 'drw') accessoriesTableBody.push(['Meerprijs achterwielen', 'Dubbellucht', { text: `€ ${P.van.drw},-`, alignment: 'right', fontSize: 10 }]);
    }

    if (model.vanstep) {
        let vanstepPrice = P.vanstep.base;
        let desc = t('vanstep');
        if (model.vanstep.towbar) {
            vanstepPrice = P.vanstep.base_towbar;
            desc += ` (${t('voorbereiding_trekhaak')})`;
        }
        accessoriesTableBody.push([desc, `${t('kleur')} ${model.vanstep.color.color}`, { text: `€ ${vanstepPrice},-`, alignment: 'right', fontSize: 10 }]);

        if (model.vanstep.color.color === 'black' || model.vanstep.color.color === 'blackSanded') {
            accessoriesTableBody.push(['', 'Kleur toeslag', { text: `€ ${P.vanstep.color_surcharge},-`, alignment: 'right', fontSize: 10 }]);
        }

        if (model.vanstep.lights) {
            accessoriesTableBody.push(['', t('verlichting'), { text: `€ ${P.vanstep.lights},-`, alignment: 'right', fontSize: 10 }]);
        }

        if (model.vanstep.mounting) {
            accessoriesTableBody.push(['', 'Montage Vanstep', { text: `€ ${P.vanstep.mounting},-`, alignment: 'right', fontSize: 10 }]);
        }

        if (model.vanstep.towbar) {
            accessoriesTableBody.push(['', 'Omkeuren naar 3500kg', { text: `€ ${P.vanstep.towbar.inspection},-`, alignment: 'right', fontSize: 10 }]);

            if (model.vanstep.towbar.type === 'standard') {
                accessoriesTableBody.push(['', t('trekhaakkogel'), { text: `€ ${P.vanstep.towbar.standard},-`, alignment: 'right', fontSize: 10 }]);
            } else if (model.vanstep.towbar.type === 'catchJaw') {
                accessoriesTableBody.push(['', t('vangmuil'), { text: `€ ${P.vanstep.towbar.catchJaw},-`, alignment: 'right', fontSize: 10 }]);
            }
            if (model.vanstep.towbar.variobloc) {
                accessoriesTableBody.push(['', t('wisselplaat'), { text: `€ ${P.vanstep.towbar.variobloc},-`, alignment: 'right', fontSize: 10 }]);
            }
        }
    }
    if (model.sidebars) {
        let sbPrice = P.sidebars.chrome;
        let sbDesc = t('sidebars') + ' ' + t('chroom');
        if (model.sidebars.color === 'black') {
            sbPrice = P.sidebars.black;
            sbDesc = t('sidebars') + ' ' + t('zwart');
        } else if (model.sidebars.color === 'luxury') {
            sbPrice = P.sidebars.luxury;
            sbDesc = t('sidebars') + ' ' + t('zwart') + ' (' + t('luxe') + ')';
        }
        accessoriesTableBody.push([sbDesc, '', { text: `€ ${sbPrice},-`, alignment: 'right', fontSize: 10 }]);
        if (model.sidebars.mounting) {
            accessoriesTableBody.push(['', 'Montage Sidebars', { text: `€ ${P.sidebars.mounting},-`, alignment: 'right', fontSize: 10 }]);
        }
    }
    if (model.sunvisor) {
        let svPrice = P.sunvisor.standard;
        let svDesc = t('zonneklep');
        if (model.sunvisor.lights) {
            svPrice = P.sunvisor.led;
            svDesc += ' met LED';
        }
        accessoriesTableBody.push([svDesc, '', { text: `€ ${svPrice},-`, alignment: 'right', fontSize: 10 }]);
        if (model.sunvisor.mounting) {
            accessoriesTableBody.push(['', 'Montage Zonneklep', { text: `€ ${P.sunvisor.mounting},-`, alignment: 'right', fontSize: 10 }]);
        }
    }
    if (model.stair) {
        let stairPrice = P.stair.small;
        let stairDesc = t('inklapbare_trap') + ' (' + t('klein') + ')';
        if (model.stair.height !== 'H1') {
            stairPrice = P.stair.large;
            stairDesc = t('inklapbare_trap') + ' (' + t('groot') + ')';
        }
        accessoriesTableBody.push([stairDesc, '', { text: `€ ${stairPrice},-`, alignment: 'right', fontSize: 10 }]);
        if (model.stair.mounting) {
            accessoriesTableBody.push(['', 'Montage Trap', { text: `€ ${P.stair.mounting},-`, alignment: 'right', fontSize: 10 }]);
        }
    }
    // --- Einde tabelbouw ---
    ...
    var docDefinition = {
        ...
        content: [
            ...
            { text: t('offerte'), font: 'Poppins', fontSize: 26, characterSpacing: 1.5, color: '#dfdeda', absolutePosition: { x: 28, y: 60 } },
            {
                text: [
                    { text: t('scan_qr_of') },
                    { text: t('klik_hier'), link: configuratorUrl, decoration: 'underline' },
                    { text: t('om_te_configureren') },
            ...
            { text: t('specificaties_voertuig'), bold: true, fontSize: 12, margin: [0, 15, 0, 5] },
            {
                layout: 'noBorders',
                table: {
                    headerRows: 0,
                    ...
                    body: [
                        [
                            { text: t('merk'), fontSize: 10, bold: true },
                            { text: `${model.van.brand.charAt(0).toUpperCase() + model.van.brand.slice(1)}`, fontSize: 10 },
                            { text: t('type'), fontSize: 10, bold: true },
                            { text: `${model.van.type.toUpperCase()}`, fontSize: 10 }
                        ],
                        [
                            { text: t('aandrijving'), fontSize: 10, bold: true },
                            { text: `${model.van.drive.toUpperCase()}`, fontSize: 10 },
                            { text: t('achterwielen'), fontSize: 10, bold: true },
                            { text: `${model.van.rearWheel === 'srw' ? t('enkellucht') : t('dubbellucht')}`, fontSize: 10 }
                        ]
                        ,
                        [
                            { text: t('kenteken_placeholder'), fontSize: 10, bold: true },
                            { text: `${model.van.licensePlate || ''}`, fontSize: 10 },
                            { text: t('chassis_placeholder'), fontSize: 10, bold: true },
                            { text: `${model.van.chassisNumber || ''}`, fontSize: 10 }
                        ]
                    ]
                },
                margin: [0, 5, 0, 0]
            ...
            { text: t('gekozen_accessoires'), bold: true, fontSize: 12, margin: [0, 15, 0, 5] },
            {
                layout: 'noBorders',
                table: {
                    headerRows: 1,
            ...
            },
            {
                ...
                table: {
                    ...
                    widths: [100, 'auto'],
                    body: [ // Totaalprijs
                        [{ text: t('prijs'), fontSize: 12, bold: true }, { text: `${price}`, fontSize: 12, bold: true }],
                ...
                columns: [
                    {
                        width: 240,
                        alignment: 'left',
                        text: t('offerte_gegenereerd'), link: 'http://tripledesign.nl', fontSize: 8
                    },
                    {
                        width: '*',
                        alignment: 'right',
    ...
    pdfMake.fonts = {
        Poppins: {
            normal: 'https://vanstep-vanstep.web.app/fonts/Poppins-Black.ttf',
            bold: 'https://vanstep-vanstep.web.app/fonts/Poppins-Black.ttf',
            tekst: 'https://vanstep-vanstep.web.app/fonts/Roboto-Light.ttf',
            //bold: 'https://vanstep-vanstep.web.app/fonts/Roboto-Light.ttf',
        },
        RobotoDefault: {
            normal: 'https://vanstep-vanstep.web.app/fonts/Roboto-Light.ttf',
            bold: 'https://vanstep-vanstep.web.app/fonts/Roboto-Medium.ttf'
            //italics: 'https://vanstep-vanstep.web.app/fonts/Roboto-Light.ttf',
            //bold: 'https://vanstep-vanstep.web.app/fonts/Roboto-Light.ttf',
        },
    }
